...
export default defineComponent({
  ...
  setup(props, { emit }) {
    ...
    // Variable
    const complaint_types = useComplaintTypeData().complaint_types;
    const complaint_type = ref({ id: 1, name_th: "ร้องเรียน" });
    const r = (Math.random() + 1).toString(36).substring(7);

    if (props.complaint_type_id) {
      complaint_type.value = complaint_types.find((x: any) => {
        return x.id == props.complaint_type_id;
      });
    }
