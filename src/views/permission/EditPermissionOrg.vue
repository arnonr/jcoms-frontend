  ...
  setup(props, context) {
    ...
    const fetchItem = async () => {
      const { data } = await ApiService.query(
        "organization-permission/" + props.id,
        {}
      );

      Object.assign(item, data.data);
      item.role_id = { id: data.data.role_id, name_th: data.data.role.name_th };

      if (data.data.role_id == 5) {
        selectOptions.value.source_orgs = [...selectOptions.value.inspectors];
        selectOptions.value.resp_orgs = [...selectOptions.value.bureaus];

        item.source_org = selectOptions.value.source_orgs.find((x: any) => {
          return x.id == data.data.inspector_id;
        });
        item.resp_org = selectOptions.value.resp_orgs.find((x: any) => {
          return x.id == data.data.resp_bureau_id;
        });
      }
      ...
      if (data.data.role_id == 3) {
        ...
        item.source_org = selectOptions.value.source_orgs.find((x: any) => {
          return x.id == data.data.bureau_id;
        });
        item.resp_org = selectOptions.value.resp_orgs.find((x: any) => {
          return x.id == data.data.resp_division_id;
        });
      }

      if (data.data.role_id == 4) {
        selectOptions.value.source_orgs = [...selectOptions.value.divisions];
        selectOptions.value.resp_orgs = [...selectOptions.value.agencys];

        item.source_org = selectOptions.value.source_orgs.find((x: any) => {
          return x.id == data.data.division_id;
        });
        item.resp_org = selectOptions.value.resp_orgs.find((x: any) => {
          return x.id == data.data.resp_agency_id;
        });
      }
    };
    ...
    const onSavePermissionOrg = async () => {
      ...
      await ApiService.put("/organization-permission/" + props.id, {
        ...data_item,
      })
        .then(({ data }) => {
          if (data.msg != "success") {
