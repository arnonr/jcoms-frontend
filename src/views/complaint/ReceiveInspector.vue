        ...
        <div class="modal-content">
          <div class="modal-header" v-if="item.id != null">
            <h3 class="modal-title">
              กองตรวจราชการ รับเรื่อง ({{ item.jcoms_no }})
            </h3>
            <button
...
export default defineComponent({
  name: "receive-inspector-complaint",
  props: {
    complaint_id: {
      type: Number,
      required: true,
    ...
    const fetchComplaintForward = async () => {
      try {
        const { data } = await ApiService.query("complaint-forward/", {
          params: { complaint_id: props.complaint_id, inspector_state_id: 1 },
        });
