        ...
        <div class="modal-content">
          <div class="modal-header" v-if="item.id != null">
            <h3 class="modal-title">
              บช./ภ. รับเรื่องติดตาม ({{ item.jcoms_no }})
            </h3>
            <button
...
export default defineComponent({
  ...
  setup(props, { emit }) {
    ...
    const fetchComplaintFollow = async () => {
      try {
        const { data } = await ApiService.query("complaint-follow/", {
          params: { complaint_id: props.complaint_id, receive_status: 1 },
        });
    ...
    const onSaveComplaint = async () => {
      ...
      await ApiService.putFormData("complaint-follow/" + item.id, data_item)
        .then(async ({ data }) => {
          if (data.msg != "success") {
